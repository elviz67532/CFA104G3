			...
			<div class="row gx-4 gx-lg-5 align-items-start">
				<div class="col-md-6">
					<img class="img-fluid rounded mb-5 mb-lg-0 img-thumbnail"
						...
						alt="" />
					<div class="mb-1">商品編號: ${productVO.id}</div>
				</div>
				<div class="col-md-6 align-items-start">
					...
					<div class="fs-4 mb-4">
						<span style="font-size: 14px;">定價: </span><span
							class="text-decoration-line-through" style="font-size: 18px;">$99999.00</span><br>
						<span style="font-size: 18px;">優惠價: </span><span
							style="font-size: 20px; color: #E60000;"><strong>
								${productVO.price}.00 </strong></span>
					</div>
					<div>
						<em
							style="color: white; font-size: 14px; background-color: #8ab70f; border-radius: 4px;">滿額優惠
						</em> &nbsp&nbsp<span
					...
					<div style="padding: 10px;">
						...
						<FORM METHOD="post"
							ACTION="${pageContext.request.contextPath}/front_end/product/productorder.do">
							<input type="hidden" name="action" value="form"> <input
								type="hidden" name="productId" value="${productVO.id}">
							<button class="btn1Buy" type="submit">
								<i class="bi-cart-fill me-1"></i> 直接購買
							</button>
						</FORM>
              ...
              <input type="hidden" name="id" value="${productVO.id}">
						...
						<a
							href="<%=request.getContextPath()%>/front_end/product/productReport.jsp?prodId=${productVO.id}">
							<button class="btn3Return" type="button">
								<i class="bi bi-people-fill"></i> 問題回報
							</button>
						</a>
					...
					<div>
						<span class="spanLocation">付款方式 : 信用卡( 限台灣發行 )</span><br> <span
							class="spanLocation">運送方式 : 臺灣與離島</span><br> <span
							class="spanLocation">可配送地點 : 台灣、蘭嶼、綠島、澎湖、金門、馬祖</span><br> <span
							class="spanLocation">可取貨地點 : 台灣、蘭嶼、綠島、澎湖、金門、馬祖</span><br> <span
							class="spanLocation">商品所在地 : ${productVO.location}</span>
					</div>
				</div>
			</div>
		</div>
		...
		<div style="margin: 0 230px;">
			<div>
				<div class="aboutProductWord"
					...
					商品詳圖</div>
				<br> <img class="img-fluid rounded mb-2 mb-lg-0 img-thumbnail"
					src="<%=request.getContextPath()%>/product_photo/DBGifReader2?prodId=${productVO.id}"
					alt="${productVO.name}" />
			</div>
			<div class="aboutProductWord"
				...
				產品說明</div>
			<div>
				<p class="lead">${productVO.description}</p>
			</div>
			<div class="small mb-1" style="text-align: center; margin-top: 20px;">
				上架時間: ${productVO.launchedDate}</div>
			<!--         </div> -->
		</div>
	</section>
