...
<meta charset="UTF-8">
<title>委域檢舉活動使用reportActivity.jsp</title>
    ...
    <header class="masthead" style="background-image: url('<%=request.getContextPath()%>/asset/img/activity01.jpg')">
   	...
   	<div style="border: 2px white groove; width: 70%; margin: 0 auto 60px auto;">
		...
		<h2>進來檢舉活動囉!</h2>
			...
			<form action="${pageContext.request.contextPath}/activity/actr.do" enctype="multipart/form-data" method="post" name="form1">
			<label class="formLabel" for="content">檢舉內容: <span style="color: red">${errorMsgs.comment}</span></label>
			<textarea class="actFormInput actContentFormInput" cols="55" name="content"><%= (actrVO == null) ? 
					"空想食境Fantasy MEALity 為 Manga'Z 所打造出的獨特餐飲體驗，將餐飲結合虛擬實境，用120分鐘的時間帶消費者走入空想王國體驗超乎想像的美食饗宴。" 
																			 :actrVO.getContent() %></textarea>
			<label class="formLabel" for="photo">活動照片: <span style="color: red">${errorMsgs.photo}</span></label>
			<input type="file" name="photo" size="45" id="flie" onchange="show(this)"/>
																			 
			<div style="padding: 0 0 0 220px;">
				<input type="submit" class="btn-hover color-5" value="送出檢舉"/>
				<input type="hidden" name="activityId"  value="${actrVO.activityId}">
				<input type="hidden" name="memberId"  value="${actrVO.memberId}">
				<input type="hidden" name="content"  value="${actrVO.content}">
 			     <input type="hidden" name="status"  value="0">
 			     <input type="hidden" name="photo"  value="${actrVO.photo}">
