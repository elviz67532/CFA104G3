...
List<MemberVO> list = memSvc.selectAll();
		...
		<div id="content-wrapper" class="d-flex flex-column">
			<div id="content">
				...
				<div class="div1">
					...
					<FORM METHOD="post" ACTION="BackMemberServlet.do">
						<b>請輸入會員編號:</b><br> <input type="text" name="id"> 
						<input type="hidden" name="action" value="getOne_For_Display"> 
						<input type="submit" value="送出" style="background-color: #4e73df; color: white; text-align: center; border: 2px solid #4e73df;">
					</FORM>
					<FORM METHOD="post" ACTION="BackMemberServlet.do">
						<b>請輸入會員姓名:</b><br> <input type="text" name="name">
					...
					<table>
						<thead>
							...
							<th class="text-nowrap">會員郵件</th>
							<th class="text-nowrap">會員帳號</th>
							<th class="text-nowrap">會員密碼</th>
							<th class="text-nowrap">會員暱稱</th>
							<th class="text-nowrap">會員姓名</th>
							<th class="text-nowrap">會員電話</th>
							<th class="text-nowrap">註冊時間</th>
							<th class="text-nowrap">會員狀態</th>
...
<b>●符 合 查 詢 條 件 如 下 所 示: 共<font color=red><%=rowNumber%></font>筆</b>
						...
						<c:forEach var="memberVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
							<tr>
								...
								<td>
									<FORM METHOD="post"ACTION="<%=request.getContextPath()%>/back_end/member/BackMemberServlet.do"style="margin-bottom: 0px;">
										<input type="submit" value="修改會員狀態"> 
										<input type="hidden" name="id" value="${memberVO.id}"> 
										<input type="hidden" name="action" value="getOne_For_Member__Back_Update">
									</FORM>
								</td>
						</c:forEach>
						</tbody>
